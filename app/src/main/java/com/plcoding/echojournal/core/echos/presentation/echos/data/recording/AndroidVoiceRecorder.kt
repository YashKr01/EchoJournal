package com.plcoding.echojournal.core.echos.presentation.echos.data.recording

import android.content.Context
import android.media.MediaRecorder
import android.os.Build
import com.plcoding.echojournal.core.echos.presentation.echos.domain.recording.RecordingDetails
...
class AndroidVoiceRecorder(
    ...
    override fun start() {
        ...
        try {
            ...
            Timber.d(e, "Failed to start recording")
            recorder?.release()
            recorder = null
        }
    }
    ...
    private fun cleanup() {
        Timber.d("Releasing voice recorder resources")
        recorder = null
        isRecording = false
        isPaused = false
        durationJob?.cancel()
    ...
    private fun startTrackingAmplitudes() {
        amplitudeJob = applicationScope.launch {
            while (isRecording) {
                ...
                withContext(singleThreadDispatcher) { amplitudes.add(amplitude) }
                delay(100L)
            }
        }
    }
    ...
    private fun getAmplitude(): Float {
        return if (isRecording) {
            try {
                ...
                maxAmplitude?.takeIf { it > 0f }?.run {
                    (this / MAX_AMPLITUDE_VALUE.toFloat()).coerceIn(0f, 1f)
                } ?: 0f
            } catch (e: Exception) {
                Timber.d(e, "Failed to retrieve current amplitude")
                0f
            }
        } else 0f
    }
